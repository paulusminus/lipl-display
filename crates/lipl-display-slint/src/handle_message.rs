    ...
    move |message| {
        match message {
            Message::Part(part) => {
                ...
                if let Err(error) = invoke_from_event_loop(move || handle_copy.unwrap().set_part(part.into())) {
                    error!("Error handling received part {}", error);
                };
            },
            Message::Status(status) => {
                let handle_copy = ui_handle.clone();
                if let Err(error) = invoke_from_event_loop(move || handle_copy.unwrap().set_status(status.into())) {
            ...
            Message::Command(command) => {
                match command {
                    Command::Dark => {
                        let handle_copy = ui_handle.clone();
                        if let Err(error) = invoke_from_event_loop(move || handle_copy.unwrap().set_dark(true)) {
                            error!("Error handling set theme dark {}", error);
                        };
                    },
                    Command::Light => {
                        let handle_copy = ui_handle.clone();
                        if let Err(error) = invoke_from_event_loop(move || handle_copy.unwrap().set_dark(false)) {
