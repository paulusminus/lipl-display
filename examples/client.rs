...
use lipl_display_common::{
    CHARACTERISTIC_COMMAND_UUID, CHARACTERISTIC_STATUS_UUID, CHARACTERISTIC_TEXT_UUID, SERVICE_UUID,
};
use zbus::{
...
async fn get_characteristic<'a>(connection: &'a Connection, path: &'a str) -> zbus::Result<characteristic::GattCharacteristic1Proxy<'a>> {
    characteristic::GattCharacteristic1Proxy::builder(connection)
...
fn get_object_paths(
    objects: &Objects,
    interface_name: &str,
) -> HashMap<OwnedObjectPath, HashMap<String, OwnedValue>> {
    objects
        .into_iter()
...
fn get_device(objects: &Objects, address: &str) -> Option<OwnedObjectPath> {
    get_object_paths(objects, "org.bluez.Device1")
        .into_iter()
        .filter(|item| {
    ...
    for (key, value) in get_object_paths(&objects, "org.bluez.Device1").iter() {
        ...
        if address == device_address.to_owned() {
            ...
            for (key, value) in get_object_paths(&objects, "org.bluez.GattService1").iter() {
                ...
                let service_device: String = OwnedObjectPath::try_from(value["Device"].clone())
                    .unwrap()
                    .to_string();
                ...
                if service_device == device && uuid == SERVICE_UUID.to_string() {
                    ...
                    for (key, value) in
                        get_object_paths(&objects, "org.bluez.GattCharacteristic1").iter()
                    {
                        let characteristic = key.to_string();
                        let uuid: String = value["UUID"].clone().try_into().unwrap();
                        let service_object_path =
                            OwnedObjectPath::try_from(value["Service"].clone())
                                .unwrap()
                                .to_string();
...
async fn main() -> zbus::Result<()> {
    ...
    if let Some(device_object_path) = get_device(&objects, LIPL) {
        let device_proxy = Device1Proxy::builder(bluez.connection())
            .destination("org.bluez")?
        ...
        if !connected {
            ...
            let mut stream = device_proxy
                .receive_services_resolved_changed()
                .await
                .boxed();
        ...
        for (uuid, path) in get_characteristic_paths(&objects, LIPL).iter() {
            let proxy = get_characteristic(bluez.connection(), path.as_str()).await?;
            ...
            if uuid == &CHARACTERISTIC_STATUS_UUID.to_string().to_lowercase() {
                ...
                proxy.write_value("Oh kindeke klein (1 / 5)".as_bytes(), HashMap::new()).await?;
            }

            if uuid == &CHARACTERISTIC_COMMAND_UUID.to_string().to_lowercase() {
                log::info!("Command Uuid Path: {path}");
                proxy.write_value("l".as_bytes(), HashMap::new()).await?;
            }
        }
    }
